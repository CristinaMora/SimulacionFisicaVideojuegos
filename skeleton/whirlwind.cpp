...
void whirlwind::updateForce(Particle* particle, double t) {
	...
	//if ((pp.x >= porigen.x && pp.x <= porigen.x + size.x) && (pp.y >= porigen.y && pp.y <= porigen.y + size.y) && (pp.z >= porigen.z && pp.z <= porigen.z + size.z)) {
		if (fabs(1 / particle->masa) < 1e-10) return;
			Vector3 velocidadtorvellino = _k1 * Vector3(-(pp.z - porigen.z), 50 - (pp.y - porigen.y), pp.x - porigen.x);
...
whirlwind::whirlwind(Vector3 fuerza, const float k, Vector3 WhirlPos, Vector3 tam_): WindofChangeForceGenerator( fuerza, WhirlPos, tam_, k, 0) {
